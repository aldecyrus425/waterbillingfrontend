...
const CashierLayout: React.FC<CashierLayoutProps> = ({ children }) => {
  ...
  const menuItems: MenuItem[] = [
    ...
    { label: "Cashiering", icon: "💵", path: "/cashier/cashiering" },
    { label: "Online Payments", icon: "🌐", path: "/cashier/online" },
    { label: "Consumers", icon: "👥", path: "/cashier/consumers" },
  ];
  ...
  const currentLabel =
    menuItems.find((item) => item.path === location.pathname)?.label || "Cashier Panel";

  return (
    <div className="flex h-screen bg-slate-50 text-slate-900">
      <div
        className={`${sidebarOpen ? "w-64" : "w-20"} bg-slate-900 text-white transition-all duration-300 shadow-xl relative`}>
          ...
          <div className="flex items-center gap-3">
            ...
            {sidebarOpen && (
              <div>
                ...
                <p className="text-xs text-slate-300">Cashier Panel</p>
              </div>
            )}
          </div>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-4 px-4 py-3 rounded-lg transition-colors group ${
                  isActive ? "bg-emerald-500 text-white" : "hover:bg-slate-700 text-slate-200"
                }`
              }
            >
          ...
          <div className="flex items-center justify-between h-20 px-6 md:px-8">
            ...
            <div className="flex items-center gap-5">
              <button className="relative text-slate-600 hover:text-slate-900 text-2xl">
                🔔
                <span className="absolute -top-1 -right-2 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">2</span>
              </button>
              ...
              <div className="relative">
                ...
                >
                  <div className="w-10 h-10 bg-gradient-to-br from-emerald-500 to-slate-700 rounded-full flex items-center justify-center text-white font-bold">
                  ...
                  <span className={`text-slate-600 transition-transform ${profileOpen ? "rotate-180" : ""}`}>▼</span>
                </button>
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                    ...
                    <div className="py-2">
                      <Link to="/" className="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 transition-colors">
                        <span>👤</span>
