...
const ConsumerLayout: React.FC<ConsumerLayoutProps> = ({ children }) => {
  ...
  const menuItems: MenuItem[] = [
    { label: "Dashboard", icon: "📊", path: "/consumer/dashboard" },
    { label: "Payment History", icon: "📜", path: "/consumer/history" },
    { label: "Pending Bills", icon: "⏳", path: "/consumer/pending" },
    { label: "Pay Bill", icon: "💳", path: "/consumer/pay" },
  ];
  ...
  const currentLabel =
    menuItems.find((item) => item.path === location.pathname)?.label || "Consumer Panel";
      ...
      <div
        ...
        <div className="h-20 flex items-center justify-center border-b border-slate-700 px-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-cyan-500 rounded-lg flex items-center justify-center text-xl font-bold">💧</div>
            {sidebarOpen && (
              <div>
                <h1 className="font-bold text-lg">WaterBill</h1>
                <p className="text-xs text-slate-300">Consumer Panel</p>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="relative">
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                    ...
                    <div className="py-2">
                      <Link to="/consumer/profile" className="flex items-center gap-3 px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 transition-colors">
                        <span>👤</span>
